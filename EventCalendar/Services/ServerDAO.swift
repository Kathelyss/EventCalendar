...
class ServerDAO {
    ...
    func requestUser(success: @escaping (UserModel) -> Void, failure: @escaping (Error) -> Void) {
        server.getData(for: "getUser", success: { (data) in
            do {
                let userModel = try self.decoder.decode(UserModel.self, from: data)
                success(userModel)
            } catch {
                failure(error)
            }
...
//    func createUser(success: @escaping (UserModel) -> Void, failure: @escaping (Error) -> Void) {
//        server.getData(for: "postUser", success: { (data) in
//            do {
//                let userModel = try self.decoder.decode(UserModel.self, from: data)
//                success(userModel)
//            } catch {
